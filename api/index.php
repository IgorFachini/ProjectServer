    ...
    try {
        ...
        if (strlen(trim($username))>0 && strlen(trim($password))>0 && strlen(trim($email))>0 && $emain_check>0 && $username_check>0 && $password_check>0)
        {
            ...
            if($mainCount==0)
            {
                ...
                $sql1="INSERT INTO users(username,password,email,name,userType,permissionType,address,latLng,complement)VALUES(?,?,?,?,?,?,?,?,?)";
                $stmt1 = $db->prepare($sql1);
                $password=hash('sha256',$data->password);
                $stmt1->execute(array($username,$password,$email,$name,$data->userType,$data->permissionType,$data->address,$data->latLng,$data->complement));
